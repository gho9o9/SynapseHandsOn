...
Azure Synapse Analytics は外部の Azure リソースの機能と連携し役割を強化することができます。第5章では Azure Purview と連携し、Synapse Pipeline によるパイプライン処理を監視しデータの系譜を記録管理します。  
それではまず Synapse と Azure Purview を連携させましょう。Synapse ワークスペースの Azure Portal メニュを辿り Synapse Studio を開きます。  
...
パイプラインを構成するにはそのパイプラインに関連するデータを「統合データセット」として Azure Synapse Analytics 内に定義します。また、統合データセットの定義にはそのソースとなるリソースを「リンクサービス」として定義する必要があります。  
...
Synapse Studio メニューを辿り「＋新規」をクリックします。  
...
| 項目 | 値 |
| :---- | :---- |
| 名前 | リンク対象の Azure Blob Storage について Azure Synapse Analytics 内で識別する際の任意の名前を指定します（例：blobstorage1130） |
...
同じく Synapse Studio メニューを辿り「＋新規」をクリックします。  
![](images/SynapseTechBook_2022-01-25-08-34-47.png)  
...
| 項目 | 値 |
| :---- | :---- |
| 名前 | リンク対象の Azure SQL Database について Azure Synapse Analytics 内で識別する際の任意の名前を指定します（例：mssqlserver1130） |
| アカウントの選択方法 | Azure サブスクリプションから |
| サブスクリプション | ハンズオンに利用しているサブスクリプションを指定します |
...
Azure Blob Storage 上の生データ（タクシー運行データ）の統合先とする Data Lake Storage 上にディレクトリを作成します。  

ストレージアカウント の Azure Portal メニュ から Synapse デプロイ時に指定したファイルシステム（例：synapsefs）の配下にディレクトリ 「raw/nyctaxidata」を作成してください。  
...
次にハンズオンアセット一式内の「resource/script/dedicatedsql.sql」の内容をエディタ上に張り付け実行をクリックします。  
...
「パイプライン実行」ブレードで「OK」をクリックするとパイプラインの実行が開始されます。  
...
Synapse Studio メニューを辿り出力先に指定した Data Lake Storage 上のパスに CSV ファイルがコピーされていることを確認します。  
...
生データと参照データを結合することで生データの情報を補完します。  
...
| 集計の設定 | グループ化 | PickUpDate, PaymentType, Borough, Zone | 集約対象列（SQL の GROUP BY 列に相当）を指定します |
| 集計の設定 | 集約：TotalTripCount | count() | 集約関数を指定します |
| 集計の設定 | 集約：TotalPassengerCount | sum(PassengerCount) | 集約関数を指定します |
| 集計の設定 | 集約：TotalDistanceTravelled | sum(DistanceTravelled) | 集約関数を指定します |
| 集計の設定 | 集約：TotalTipAmount | sum(TipAmount) | 集約関数を指定します |
| 集計の設定 | 集約：TotalFareAmount | sum(FareAmount) | 集約関数を指定します |
| 集計の設定 | 集約：TotalTripAmount | sum(TotalAmount) | 集約関数を指定します |
