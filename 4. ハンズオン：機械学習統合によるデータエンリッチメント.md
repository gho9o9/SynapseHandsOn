...
このハンズオンでは Azure Synapse Analytics から Azure Machine Learning への認証に マネージド ID を利用するものとし、Azure Synapse Analytics の マネージドID に対して Azure Machine Learning の 共同作成者ロールを付与します。  
...
Synapse ワークスペースの Azure Portal メニュを辿り Synapse Studio を起動します。  
...
| 項目 | 値 |
| :---- | :---- |
| 名前 | リンク対象の Azure Machine Learning について Azure Synapse Analytics 内で識別する際の任意の名前を指定します（例：MyAML） |
| 認証方法 | マネージドID |
...
これまでの定義を発行し確定します。「すべてを発行」->「発行」の順でボタンをクリックします。  
...
AutoML の実験の Status が「Completed」となるのをしばらく待ちます。  
![](images/SynapseTechBook_2022-02-25-23-56-26.png)  

「実験」メニュの「モデル」タブを開くと AutoML によって作成された機械学習モデルの一覧を確認することができます。  
![](images/SynapseTechBook_2022-04-12-16-39-15.png)  
***TODO：画像あってる？***

「モデル」メニュを開くと作成された機械学習モデル（AutoML によって作成された機械学習モデルの中で最も成績の良いモデル）が登録されていることが確認することができます。  
...
***TODO；画像が無い ＆ もう少し内容を補足***

GUI で構成した情報をもとにしたビュー定義とビュー参照が記述された SQL スクリプトが起動します。  
...
構成は Azure Portal からもできますが、今回はコマンドで設定します。Azure Portal メニュから Cloud Shell を起動し Bash モードを選択します。  
...
Synapse Studio メニュを辿り「＋新規」を選択します。  
![](images/SynapseTechBook_2022-02-25-18-17-51.png)
...
| 項目 | 値 |
| :---- | :---- |
| 名前 | リンク対象の Azure Key Vault について Azure Synapse Analytics 内で識別する際の任意の名前を指定します（例：MyKey） |
| 認証方法 | マネージドID |
| Azure サブスクリプション | ご利用のサブスクリプションを指定します |
...
これまでの定義を発行し確定します。「すべてを発行」、「発行」の順でボタンを押下します。  
![](images/SynapseTechBook_2022-02-25-18-26-52.png)  
...
| 項目 | 値 |
| :---- | :---- |
| 名前 | リンク対象の Azure Cognitive Services Anomaly Detector について Azure Synapse Analytics 内で識別する際の任意の名前を指定します（例：MyCogAnomaly） |
| Azure サブスクリプション | ご利用のサブスクリプションを指定します |
| Azure Cognitive Services の名前 | 作成した Azure Cognitive Services Anomaly Detector を指定します |
| AKV のリンクサービス | 前の手順で構成した Azure Key Vault のリンクサービスを指定します（例：MyKey） |
...
| 項目 | 値 |
| :---- | :---- |
| 名前 | リンク対象の Azure Cognitive Services Text Analytics について Azure Synapse Analytics 内で識別する際の任意の名前を指定します（例：MyCogSentiment） |
| Azure サブスクリプション | ご利用のサブスクリプションを指定します |
| Azure Cognitive Services の名前 | 作成した Azure Cognitive Services Text Analytics を指定します |
| AKV のリンクサービス | 前の手順で作成した Azure Key Vault のリンクサービスを指定します（例：MyKey） |
...
| コンテナ名 | Synapse デプロイ時に作成した Data Lake Storage のファイルシステム名を指定します（例：synapsefs） |  
...
Synapse Studio のメニューを辿り「モデルを使用した予測」を選択します。  
...
| 項目 | 値 |
| :---- | :---- |
| Azure Cognitive Services のリンクサービス | 前の手順で作成した Azure Cognitive Services Anomaly Detector のリンクサービスを指定します（例：MyCogAnomaly） |
| 細分性 | minutely |
| タイムスタンプ列 | dateTime |
| 時系列値 | measureValue |  
| 列のグループ化 | deviceUnit |  
...
右隅の「表示オプション」をクリックします。  
![](images/SynapseTechBook_2022-03-05-19-39-27.png)  

表示オプションを以下のように設定したのち「適用」をクリックします。
...
sentiment = (TextSentiment()
    .setLinkedService("<Azure Cognitive Services Text Analyticsのリンクサービス名を指定します（例：MyCogSentiment）>")
    .setOutputCol("output")
    .setErrorCol("error")
    .setLanguage("ja")
    .setTextCol("review"))
